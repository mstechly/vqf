...
class OptimizationEngine(object):
    ...
    def perform_qaoa(self):
        ...
        betas, gammas = self.find_initial_angles_with_interp(starting_angles=self.step_by_step_results)
    ...
    def get_angles(self):
        ...
        result, bfgs_evaluations = vqe.vqe_run(param_prog, cost_ham, stacked_params, qc=self.qaoa_inst.qc,
    ...
    def find_initial_angles_with_interp(self, starting_angles):
        ...
        if len(starting_angles[0]) == p:
            ...
            for i in range(1, p + 2):
                ...
                new_beta_i = (i - 1) / p * beta_i_minus_1 + (p - i + 1) / p * beta_i
                new_betas.append(new_beta_i)

                gamma_i = previous_gammas[i]
                gamma_i_minus_1 = previous_gammas[i-1]
                new_gamma_i = (i - 1) / p * gamma_i_minus_1 + (p - i + 1) / p * gamma_i
            ...
            Exception("INTERP strategy won't work without proper starting angles.")
